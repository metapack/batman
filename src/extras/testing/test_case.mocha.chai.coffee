class Batman.TestCase extends Batman.Object
  ...
  runTests: ->
    describe @constructor.name, =>
      beforeEach @xhrWrapper(@setup.bind(this))
      afterEach @xhrWrapper(@teardown.bind(this))
  ...
  assert: (assertion, message = 'was not true') ->
    chai.assert.ok assertion, message

  assertEqual: (expected, actual, message) ->
    chai.assert.equal expected, actual, message or "Expected: #{expected} \nGot: #{actual}"

  assertDeepEqual: (expected, actual, message) ->
    chai.assert.deepEqual expected, actual, message

  assertNotEqual: (expected, actual, message) ->
    chai.assert.notEqual expected, actual, message or "Value not expected to match: #{expected}"

  assertMatch: (expected, actual, message) ->
    chai.assert.match expected,actual, message or "Expected: #{expected} \nGot: #{actual}"

  assertNoMatch: (expected, actual, message) ->
    chai.assert.notMatch expected, actual, message or "Value not expected to match: #{expected}"
  ...
  assertRaises: (expected, callback, message) ->
    chai.assert.throws callback, expected, message
