...
exports.extend = extend = (to, objects...) ->
...
exports.unmixin = unmixin = (from, mixins...) ->
  for mixin in mixins
    for key of mixin
...
_implementImmediates = (container) ->
  ...
  if container.setImmediate and container.clearImmediate
    setImmediate = -> container.setImmediate.apply(container, arguments)
    clearImmediate = -> container.clearImmediate.apply(container, arguments)
...
exports.forEach = forEach = (container, iterator, ctx) ->
...
exports.objectHasKey = objectHasKey = (object, key) ->
  if typeof object.hasKey is 'function'
    object.hasKey(key)
  else
    key of object

exports.contains = contains = (container, item) ->
...
exports.escapeHTML = escapeHTML = do ->
  return (s) -> (""+s).replace(_unsafeCharsPattern, (c) -> _entityMap[c])

exports.unescapeHTML = unescapeHTML = do ->
  return (s) ->
    return unless s?
    node = _unescapeHTMLNode ||= document.createElement('DIV')
    node.innerHTML = s
    Batman.DOM.textContent(node)

# `initializeObject` is called by all the methods in Batman.Object to ensure that the
...
# initialization.
exports.initializeObject = initializeObject = (object) ->
  if object._batman?
